...
class RobotDriver:
    ...
    def emergency_disable_torque(self):
        """Emergency disable torque immediately without any other operations."""
        if not self.connected or not self.portHandler:
            return
        
        print("⚠️  EMERGENCY: Disabling torque immediately...")
        for dxl_id in robot_config.motor_ids:
            try:
                self.packetHandler.write1ByteTxRx(
                    self.portHandler, dxl_id, robot_config.addr_torque_enable, 0
                ...
                pass
        print("✓ Emergency torque disabled")
    ...
    def enable_torque_all(self):
        ...
        for dxl_id in robot_config.motor_ids:
            try:
                ...
                if dxl_comm_result != COMM_SUCCESS:
                    print(f"⚠️  Warning: Failed to enable torque on motor {dxl_id}: {self.packetHandler.getTxRxResult(dxl_comm_result)}")
                elif dxl_error != 0:
                    print(f"⚠️  Warning: Motor {dxl_id} error: {self.packetHandler.getRxPacketError(dxl_error)}")
            except Exception as e:
                print(f"⚠️  Warning: Exception while enabling torque on motor {dxl_id}: {e}")
        time.sleep(0.5)  # Wait for torque to fully enable
        print("✓ Torque enabled on all motors")
    
    def is_torque_enabled(self, motor_id):
        """Check if torque is enabled on a specific motor."""
        if not self.connected or not self.portHandler:
            return False
        
        try:
            torque_value, dxl_comm_result, dxl_error = self.packetHandler.read1ByteTxRx(
                self.portHandler, motor_id, robot_config.addr_torque_enable
            )
            from dynamixel_sdk import COMM_SUCCESS
    ...
    def check_and_reenable_torque(self):
        """Check if torque is disabled on any motor, and re-enable if needed."""
        if not self.connected or not self.portHandler:
            return False
        
        torque_disabled = False
        for dxl_id in robot_config.motor_ids:
            if not self.is_torque_enabled(dxl_id):
                torque_disabled = True
                break
        
        if torque_disabled:
            print("⚠️  Torque was disabled. Re-enabling torque on all motors...")
