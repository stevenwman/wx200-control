"""
Gym environment utilities for WX200 robot - POSITION TARGETS PROTOTYPE.

PROTOTYPE VERSION: Uses position targets instead of velocity deltas.
- Actions: [x, y, z, qw, qx, qy, qz, gripper] (8D position targets)
- Encoder feedback at every step to sync MuJoCo state
- Direct position control via reset_pose() instead of velocity integration
...
# DATASET_NAME = "merged_data_states_position_targets.npz"
...
def get_dataset(env, env_name):
    ...
    POSITION TARGETS VERSION: Actions are EE pose targets from next timestep.
    
    Expected format (from merged_data.npz):
    - smoothed_observations: (N, 20) - current observation
    - next_observations: (N, 20) - next observation (contains ee_pose_debug[t+1])
    - actions_flat: (N, 8) = [x, y, z, qw, qx, qy, qz, gripper]
        Actions should be extracted from next_observations' ee_pose_debug (staggered)
        ALWAYS in control space (unnormalized):
        - [x, y, z]: meters (workspace bounds)
        - [qw, qx, qy, qz]: unit quaternion
        - gripper: meters (range: [-0.026, 0.0])
    
    NOTE: For now, this function expects actions_flat to already contain position targets.
    The dataset compilation in check_data.ipynb needs to extract ee_pose_debug from
    next_observations and use that as actions.
    
    This function ALWAYS normalizes actions from control space to [-1, 1] for training.
    """
    ...
    print(f"  Actions are position targets in control space (unnormalized) - will normalize to [-1, 1]")
    
    # Check if actions need to be extracted from next_observations
    # If actions_flat is still velocity-based (7D), extract position targets from next_observations
    if actions.shape[1] == 7:
        print("  WARNING: actions_flat is 7D (velocity-based). Extracting position targets from next_observations...")
        # Extract ee_pose_debug from next_observations (last 7 dims are ee_pose_debug)
        # Observation structure: [aruco_obj_in_world (7D), robot_state (6D), ee_pose_debug (7D)] = 20D
        ee_pose_debug_next = next_observations[:, -7:]  # Last 7 dims are ee_pose_debug [x, y, z, qw, qx, qy, qz]
        # Note: Gripper is NOT in ee_pose_debug, it's separate. We need to get it from the original actions or compute it.
        # For now, use gripper from original actions (last dim)
        gripper_next = actions[:, -1]  # Gripper from original actions
        
        # Combine: [x, y, z, qw, qx, qy, qz, gripper] = 8D
        actions = np.concatenate([
            ee_pose_debug_next,  # [x, y, z, qw, qx, qy, qz]
            gripper_next[:, None]  # Gripper [gripper]
        ], axis=1)
        print(f"  Extracted position targets: new actions shape = {actions.shape}")
    
    # Normalize dataset actions from control ranges to [-1, 1] for training
    # Position targets: [x, y, z, qw, qx, qy, qz, gripper]
    # Position bounds from workspace
    ...
    # Quaternion bounds (unit quaternions)
    quaternion_low = np.array([-1.0, -1.0, -1.0, -1.0])
    quaternion_high = np.array([1.0, 1.0, 1.0, 1.0])
    
    # Gripper bounds
    gripper_low = robot_config.gripper_open_pos
    gripper_high = robot_config.gripper_closed_pos
    
    # Combine bounds
    action_low = np.concatenate([position_low, quaternion_low, [gripper_low]])
    action_high = np.concatenate([position_high, quaternion_high, [gripper_high]])
    
    # Normalize: (action - low) / (high - low) * 2 - 1 -> [-1, 1]
    normalized_actions = 2.0 * (actions - action_low) / (action_high - action_low) - 1.0
    
    print(f"  Normalized position targets from control ranges to [-1, 1]")
    print(f"    Position ranges: x={robot_config.ee_bound_x}, y={robot_config.ee_bound_y}, z={robot_config.ee_bound_z}")
    print(f"    Quaternion range: [-1, 1] (will be renormalized to unit quaternion)")
    print(f"    Gripper range: [{gripper_low}, {gripper_high}]")
...
class WX200GymEnvBase(gym.Env):
    ...
    def __init__(self, max_episode_length=1000, control_frequency=None, 
        ...
        # Setup spaces - POSITION TARGETS: [x, y, z, qw, qx, qy, qz, gripper] = 8D
    ...
    def _get_observation(self):
        ...
        # Match dataset observation order from check_data.ipynb:
        # [obj_world (7D), ee_poses_debug (7D), states (6D)]
        # obs = np.concatenate([
        #     self.aruco_obs_dict['aruco_object_in_world'],  # 7D
    ...
    def _denormalize_action(self, action):
        """
        Denormalize action from [-1, 1] to actual position/orientation targets.
        
        POSITION TARGETS: [x, y, z, qw, qx, qy, qz, gripper] in [-1, 1] (normalized)
        
        Control ranges:
        - Position [x, y, z]: Workspace bounds from robot_config.ee_bound_*
        - Orientation [qw, qx, qy, qz]: Unit quaternion (components in [-1, 1] range)
        - Gripper target: [-0.026, 0.0] meters (open to closed)
        
        Denormalization formula: action = (normalized_action + 1) / 2 * (high - low) + low
        
        Args:
            action: [x, y, z, qw, qx, qy, qz, gripper] in [-1, 1] (normalized)
        
        Returns:
            tuple: (target_position, target_orientation_quat_wxyz, gripper_target)
        """
        # Position bounds from workspace
        position_low = np.array([
            robot_config.ee_bound_x[0],  # x min
            robot_config.ee_bound_y[0],  # y min
            robot_config.ee_bound_z[0],  # z min
        ])
        position_high = np.array([
            robot_config.ee_bound_x[1],  # x max
            robot_config.ee_bound_y[1],  # y max
            robot_config.ee_bound_z[1],  # z max
        ])
        
        # Quaternion bounds (unit quaternions, components typically in [-1, 1])
        quaternion_low = np.array([-1.0, -1.0, -1.0, -1.0])  # [qw, qx, qy, qz]
        quaternion_high = np.array([1.0, 1.0, 1.0, 1.0])
        
        # Gripper bounds
        gripper_low = robot_config.gripper_open_pos  # -0.026
        gripper_high = robot_config.gripper_closed_pos  # 0.0
        
        # Denormalize position
        target_position = (action[:3] + 1.0) / 2.0 * (position_high - position_low) + position_low
        
        # Denormalize quaternion (then renormalize to unit quaternion)
        target_quat = (action[3:7] + 1.0) / 2.0 * (quaternion_high - quaternion_low) + quaternion_low
        # Renormalize to unit quaternion
        quat_norm = np.linalg.norm(target_quat)
        if quat_norm > 1e-6:
            target_orientation_quat_wxyz = target_quat / quat_norm
        else:
            # Fallback to identity quaternion if denormalized quaternion is invalid
            target_orientation_quat_wxyz = np.array([1.0, 0.0, 0.0, 0.0])
        
        # Denormalize gripper
        gripper_target = (action[7] + 1.0) / 2.0 * (gripper_high - gripper_low) + gripper_low
        
        return target_position, target_orientation_quat_wxyz, gripper_target
    ...
    def step(self, action):
        ...
        POSITION TARGETS VERSION: Uses encoder feedback + position targets
        
        Args:
            action: [x, y, z, qw, qx, qy, qz, gripper] in [-1, 1] (normalized position target)
        ...
        # Clip action to valid range BEFORE denormalization (critical safety check)
        action = np.clip(action, self.action_space.low, self.action_space.high)
        
        # Additional safety: ensure action is finite (no NaN/Inf)
        if not np.all(np.isfinite(action)):
            print(f"⚠️  CRITICAL: Non-finite action detected at step {self.episode_step}: {action}")
        ...
        # DIAGNOSTIC: Check if action is hitting boundaries in normalized space
        # This helps detect if network is outputting extreme values that get clipped
        at_normalized_boundary = np.any(np.abs(action) >= 0.995)  # Very close to ±1 (likely clipped)
        
        # CRITICAL: If ALL dimensions are at boundaries, this suggests a serious problem
        # (checkpoint mismatch, network not trained, or fundamental bug)
        all_at_boundary = np.all(np.abs(action) >= 0.995)
        if all_at_boundary:
            if not hasattr(self, '_all_boundary_warning_count'):
                self._all_boundary_warning_count = 0
            self._all_boundary_warning_count += 1
            if self._all_boundary_warning_count <= 3:  # Only warn first 3 times
                print(f"⚠️  CRITICAL: ALL action dimensions at boundary (exactly ±1.0)!")
                print(f"    This suggests the checkpoint may be incompatible (wrong action space) or network is broken.")
                print(f"    Normalized action: {action}")
                print(f"    Step {self.episode_step}")
                print(f"    This will cause IK failures and unsafe robot movements!")
                if self._all_boundary_warning_count == 3:
                    print(f"    (Suppressing further warnings - checkpoint likely incompatible)")
        
        if at_normalized_boundary and not all_at_boundary:
            boundary_dims = np.where(np.abs(action) >= 0.995)[0]
            if not hasattr(self, '_boundary_action_count'):
                self._boundary_action_count = 0
            self._boundary_action_count += 1
            if self._boundary_action_count % 5 == 0:  # Print every 5th boundary action
                print(f"⚠️  Action at normalized boundary (likely clipped): dims {boundary_dims}, values {action[boundary_dims]}")
                print(f"    Step {self.episode_step}, normalized action[:3] (position): {action[:3]}")
        
        # Denormalize action to position target
        target_position, target_orientation_quat_wxyz, gripper_target = self._denormalize_action(action)
        
        # SAFETY: Soften boundary positions slightly to avoid IK failures
        # If position is very close to boundary, move it slightly inward to ensure reachability
        boundary_tolerance = 0.005  # 5mm inward from boundary
        if abs(target_position[0] - robot_config.ee_bound_x[0]) < boundary_tolerance:
            target_position[0] = robot_config.ee_bound_x[0] + boundary_tolerance
        elif abs(target_position[0] - robot_config.ee_bound_x[1]) < boundary_tolerance:
            target_position[0] = robot_config.ee_bound_x[1] - boundary_tolerance
        
        if abs(target_position[1] - robot_config.ee_bound_y[0]) < boundary_tolerance:
            target_position[1] = robot_config.ee_bound_y[0] + boundary_tolerance
        elif abs(target_position[1] - robot_config.ee_bound_y[1]) < boundary_tolerance:
            target_position[1] = robot_config.ee_bound_y[1] - boundary_tolerance
            
        if abs(target_position[2] - robot_config.ee_bound_z[0]) < boundary_tolerance:
            target_position[2] = robot_config.ee_bound_z[0] + boundary_tolerance
        elif abs(target_position[2] - robot_config.ee_bound_z[1]) < boundary_tolerance:
            target_position[2] = robot_config.ee_bound_z[1] - boundary_tolerance
        
        # Apply control using position target (skip if no hardware authority)
        # NOTE: We assume the robot continuously follows IK commands. Encoder sync happens
        # only on reset (or periodically if needed). This avoids latency from reading encoders every step.
        if self.has_authority and self._hardware_initialized and self.robot_base is not None:
            # Clamp target position to workspace bounds (safety)
            bounds_min = np.array([
                robot_config.ee_bound_x[0],
                robot_config.ee_bound_y[0],
                robot_config.ee_bound_z[0],
            ...
            # Warn if clamping actually changed the position (indicates denormalized action was outside bounds)
            if not np.allclose(target_position, clamped_position, atol=1e-6):
                if not hasattr(self, '_bounds_warning_count'):
                    self._bounds_warning_count = 0
                self._bounds_warning_count += 1
                if self._bounds_warning_count % 10 == 0:  # Warn every 10th occurrence
                    print(f"⚠️  Position target clamped at step {self.episode_step}: {target_position} -> {clamped_position}")
            
            # Set target pose directly (no velocity integration)
            self.robot_base.robot_controller.reset_pose(clamped_position, target_orientation_quat_wxyz)
            target_pose = self.robot_base.robot_controller.get_target_pose()
            self.robot_base.robot_controller.end_effector_task.set_target(target_pose)
            
            # Solve IK to get joint commands
            ik_converged = self.robot_base.robot_controller._converge_ik(
                self.robot_base.configuration,
                self.dt
            )
            
            # Warn if IK didn't converge and handle it safely
            if not ik_converged:
                # Only warn occasionally to avoid spam
                if not hasattr(self, '_ik_warning_count'):
                    self._ik_warning_count = 0
                self._ik_warning_count += 1
                if self._ik_warning_count % 10 == 0:  # Warn every 10th failure
                    print(f"⚠️  IK did not converge at step {self.episode_step} (target: {clamped_position})")
                
                # Check if target is at workspace boundary (common cause of IK failure)
                at_x_boundary = (abs(clamped_position[0] - robot_config.ee_bound_x[0]) < 1e-4 or 
                                abs(clamped_position[0] - robot_config.ee_bound_x[1]) < 1e-4)
                at_y_boundary = (abs(clamped_position[1] - robot_config.ee_bound_y[0]) < 1e-4 or 
                                abs(clamped_position[1] - robot_config.ee_bound_y[1]) < 1e-4)
                at_z_boundary = (abs(clamped_position[2] - robot_config.ee_bound_z[0]) < 1e-4 or 
                                abs(clamped_position[2] - robot_config.ee_bound_z[1]) < 1e-4)
                
                if at_x_boundary or at_y_boundary or at_z_boundary:
                    # Target is at boundary - IK failure is expected, but we should still be careful
                    # Continue with IK solution but validate it more strictly
                    pass
                else:
                    # Target is NOT at boundary but IK failed - this is more concerning
                    # Still proceed but this suggests the target might be unreachable for other reasons
                    pass
            
            # Get joint commands from IK solution (even if IK didn't fully converge, 
            # the solution might be reasonable)
            joint_commands_rad = self.robot_base.configuration.q[:5].copy()
            
            # Validate joint commands are reasonable (sanity check)
            # Check for NaN or Inf
            if np.any(np.isnan(joint_commands_rad)) or np.any(np.isinf(joint_commands_rad)):
                print(f"⚠️  CRITICAL: Invalid joint commands detected (NaN/Inf) at step {self.episode_step}!")
                print(f"    Joint commands: {joint_commands_rad}")
                print(f"    Target position: {clamped_position}")
                # Don't send invalid commands - skip this step
                obs = self._get_observation()
                self.episode_step += 1
                reward = -1.0
                terminated = False
                truncated = self.episode_step >= self.max_episode_length
                return obs, reward, terminated, truncated, {"episode_step": self.episode_step, "episode_return": self.episode_return}
            
            # Validate IK solution quality: compute forward kinematics and check position error
            # If IK didn't converge and error is large, reject the solution to avoid unsafe movements
            if not ik_converged:
                # Compute actual EE position from IK solution using forward kinematics
                actual_position, _ = self._get_current_ee_pose()
                position_error = np.linalg.norm(actual_position - clamped_position)
                
                # If error is too large (>2cm), reject the solution
                max_allowed_error = 0.02  # 2cm
                if position_error > max_allowed_error:
                    # IK solution is too far from target - skip sending commands this step
                    if not hasattr(self, '_ik_reject_count'):
                        self._ik_reject_count = 0
                    self._ik_reject_count += 1
                    if self._ik_reject_count % 5 == 0:  # Warn every 5th rejection
                        print(f"⚠️  REJECTING IK solution: error={position_error*1000:.1f}mm > {max_allowed_error*1000:.0f}mm threshold")
                        print(f"    Target: {clamped_position}, Actual: {actual_position}")
                    
                    # Don't send commands - return early
                    obs = self._get_observation()
                    self.episode_step += 1
                    reward = -1.0
                    terminated = False
            ...
            # Check if joint commands are within reasonable ranges (sanity check)
            # Typical joint limits: ~[-pi, pi] for most joints
            joint_limit = np.pi * 2.0  # Allow some margin beyond pi
            if np.any(np.abs(joint_commands_rad) > joint_limit):
                print(f"⚠️  WARNING: Joint commands exceed reasonable limits at step {self.episode_step}")
                print(f"    Joint commands: {joint_commands_rad}")
                print(f"    Target position: {clamped_position}")
                # Clamp joints to reasonable range (safety)
                joint_commands_rad = np.clip(joint_commands_rad, -joint_limit, joint_limit)
            
            # Convert joint commands to motor positions
            motor_positions = self.robot_base.translator.joint_commands_to_motor_positions(
        ...
        # Get observation
        # NOTE: ee_pose_debug is computed from configuration.q (IK solution), not encoders.
        # This assumes the robot follows IK commands reasonably well. For more accurate
        # observations, periodic encoder sync could be added (e.g., every N steps).
        obs = self._get_observation()
