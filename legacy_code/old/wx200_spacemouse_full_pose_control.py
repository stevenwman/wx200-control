...
def main():
    ...
    with mujoco.viewer.launch_passive(
        ...
        # Initialize target orientation from current site orientation
        ...
        target_quat_wxyz = np.array([current_site_quat[3], current_site_quat[0], 
        ...
        while viewer.is_running():
            ...
            if not data_queue.empty():
                try:
                    ...
                    # We want: roll (x-axis) -> wx, pitch (y-axis) -> wy, yaw (z-axis) -> wz
                    # So we need to map: [roll, yaw, pitch] -> [wx, wy, wz] = [roll, pitch, yaw]
                    rotation_twist = twist_command.get('rotation', np.zeros(3))
                    # rotation_twist is [roll, yaw, pitch] from spacemouse_reader
                    # Map to world frame angular velocity: [wx, wy, wz] = [-pitch, -roll, yaw] (x and y flipped and negated)
            ...
            omega_magnitude = np.linalg.norm(current_angular_velocity_world)
            if omega_magnitude > 1e-6:
                # Normalize angular velocity and compute rotation angle
                omega_normalized = current_angular_velocity_world / omega_magnitude
                ...
                current_rot = R.from_quat([target_quat_wxyz[1], target_quat_wxyz[2], 
                                           target_quat_wxyz[3], target_quat_wxyz[0]])  # [x, y, z, w]
                ...
                target_quat_wxyz = np.array([new_quat[3], new_quat[0], 
                                             new_quat[1], new_quat[2]])  # [w, x, y, z]
                # Normalize quaternion
                target_quat_wxyz = target_quat_wxyz / np.linalg.norm(target_quat_wxyz)
            
            # Set target for IK (position + orientation)
            target_pose = mink.SE3(np.concatenate([target_quat_wxyz, target_position]))
