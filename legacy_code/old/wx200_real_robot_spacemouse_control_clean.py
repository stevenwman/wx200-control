...
GRIPPER_INCREMENT_RATE = 0.00025  # Position change per loop when button held (m) - adjust for responsiveness
...
MOVE_DELAY = 1.0  # Seconds to wait between shutdown moves
...
def main():
    ...
    try:
        ...
        gripper_current_position = GRIPPER_OPEN_POS  # Start at open position
        ...
        try:
            ...
            while running and control_loop_active:
                ...
                t0 = time.perf_counter()
                # Get button states for incremental gripper control
                left_button_pressed, right_button_pressed = spacemouse.get_gripper_button_states()
                
                # Incrementally adjust gripper position based on button hold
                if left_button_pressed:
                    # Hold left button = open (move towards GRIPPER_OPEN_POS)
                    ...
                    # Hold right button = close (move towards GRIPPER_CLOSED_POS)
                    gripper_current_position += GRIPPER_INCREMENT_RATE
                ...
                if ENABLE_PROFILING:
                    profile_times['gripper_control'].append(time.perf_counter() - t0)
