...
JOINT_ANGULAR_VELOCITY_SCALE = 1.0  # rad/s per unit SpaceMouse z-twist input
...
def main():
    ...
    # Start spacemouse reader process (translation + z-twist for last joint)
    spacemouse_proc = mp.Process(
        target=spacemouse_process,
        args=(data_queue, VELOCITY_SCALE, JOINT_ANGULAR_VELOCITY_SCALE)  # Use rotation scale for z-twist
    )
    spacemouse_proc.start()
    print("SpaceMouse process started (Position + Last Joint Control)")
    print("Translation: Pushing forward = world +X, right = world +Y, up = world +Z")
    print("Z-twist: Controls last revolute joint (link4-5_y) angular velocity")
    ...
    with mujoco.viewer.launch_passive(
        ...
        # Last revolute joint control (link4-5_y)
        last_joint_name = "link4-5_y"
        last_joint_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_JOINT, last_joint_name)
        last_joint_angular_velocity = 0.0  # rad/s
        
        print(f"Last revolute joint: {last_joint_name} (joint_id={last_joint_id})")
        ...
        while viewer.is_running():
            ...
            current_velocity_world = np.zeros(3)
            last_joint_angular_velocity = 0.0  # Reset joint angular velocity
            ...
            if not data_queue.empty():
                try:
                    ...
                    # Extract z-twist (yaw) for last joint control
                    # rotation_twist is [roll, yaw, pitch] = [wx, wy, wz] in world frame
                    # Based on spacemouse_reader.py: rotation_twist = [roll, yaw, pitch]
                    # So rotation_twist[1] is yaw (z-twist), which we want for the last joint
                    rotation_twist = twist_command.get('rotation', np.zeros(3))
                    omega_z = rotation_twist[1]  # yaw (z-twist) angular velocity
                    
                    # Apply deadzone for joint control
                    if abs(omega_z) > 0.01:  # Deadzone threshold
                        last_joint_angular_velocity = omega_z
            ...
            # Set robot controls from IK (first 5 joints)
            data.ctrl[:5] = configuration.q[:5]
            
            # Control last revolute joint directly from SpaceMouse z-twist
            # Integrate angular velocity to get desired joint angle
            if last_joint_id >= 0:
                current_joint_angle = data.qpos[last_joint_id]
                new_joint_angle = current_joint_angle + last_joint_angular_velocity * dt
                # Clamp to joint limits (from XML: range="-3.1415926535897922 3.141592653589794")
                new_joint_angle = np.clip(new_joint_angle, -np.pi, np.pi)
                # Set control for last joint directly (joint index 4, which is ctrl index 4)
                data.ctrl[last_joint_id] = new_joint_angle
