...
class WX200GymEnv(gym.Env):
    ...
    def _fix_usb_latency(self, device='/dev/ttyUSB0', target_latency=1):
        ...
        if not os.path.exists(device):
            print(f"[WX200GymEnv] Device {device} not found, skipping latency check")
            return

        latency_path = f'/sys/bus/usb-serial/devices/{os.path.basename(device)}/latency_timer'
        ...
        try:
            ...
            if current == target_latency:
                print(f"[WX200GymEnv] USB latency already optimal ({current}ms)")
                return

            print(f"[WX200GymEnv] USB latency is {current}ms (need {target_latency}ms)")
            print(f"[WX200GymEnv] Attempting to fix with sudo...")

            result = subprocess.run(
                ['sudo', 'sh', '-c', f'echo {target_latency} > {latency_path}'],
            ...
            if result.returncode == 0:
                ...
                if new == target_latency:
                    print(f"[WX200GymEnv] USB latency fixed: {current}ms -> {new}ms")
                else:
                    print(f"[WX200GymEnv] Latency change failed, continuing anyway...")
            else:
                print(f"[WX200GymEnv] Could not fix latency (sudo failed), continuing anyway...")

        except Exception as e:
            print(f"[WX200GymEnv] Latency check failed: {e}, continuing anyway...")
    ...
    def _initialize_hardware(self, camera_id=None, width=None, height=None, fps=None):
        ...
        try:
            ...
            # Fix USB latency before initializing robot
            self._fix_usb_latency()
