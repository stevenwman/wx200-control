...
Designed for compact_gym demo files under data/gym_demos/ (demo_*.npz).
"""
...
def smooth_trajectory_file(input_path: Path, output_path: Path = None):
    ...
    Args:
        ...
        output_path: Path to output .npz file (default: adds '_smoothed' before .npz).
            Pass the same path as input_path to overwrite in-place.
    """
    ...
    if output_path == input_path:
        print(f"\nSaving smoothed trajectory in-place: {output_path}")
    else:
        print(f"\nSaving smoothed trajectory: {output_path}")
    _atomic_save_npz(output_path, **new_data)
    print("Done!")
    
    return output_path


def _atomic_save_npz(output_path: Path, **data):
    """Write NPZ atomically to avoid corrupting existing files."""
    output_path = Path(output_path)
    output_path.parent.mkdir(parents=True, exist_ok=True)

    fd, tmp_path = tempfile.mkstemp(suffix=output_path.suffix, dir=output_path.parent)
    os.close(fd)
    try:
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        # Default: process compact_gym demo folder
        data_dir = Path(__file__).parent.parent / "data" / "gym_demos"
        if data_dir.exists():
...
def process_directory(directory: Path):
    """Process all demo/trajectory files in a directory, excluding quarantined ones."""
    # Find demo_*.npz and trajectory_*.npz files, excluding quarantined
    traj_files = [
        f for f in sorted(directory.glob("demo_*.npz"))
