...
class RobotDriver:
    ...
    def reboot_motor(self, motor_id):
        ...
        try:
            ...
            # Wait longer for motor to fully reset (0.1s is often too short for WX200 motors)
            time.sleep(0.5)
    ...
    def connect(self):
        ...
        # Reboot motors only if needed (to prevent dropping the arm if it's healthy)
        # We try to enable torque first. If it fails, then we reboot that specific motor.
        ...
        for dxl_id in robot_config.motor_ids:
            ...
            # Check if failed or error occurred
            needs_reboot = False
            if dxl_comm_result != COMM_SUCCESS:
                print(f"Motor {dxl_id} comm failed: {self.packetHandler.getTxRxResult(dxl_comm_result)}")
                needs_reboot = True
            elif dxl_error != 0:
                print(f"Motor {dxl_id} hardware error: {self.packetHandler.getRxPacketError(dxl_error)}")
                needs_reboot = True
                
            if needs_reboot:
                print(f"Rebooting Motor {dxl_id} due to error...")
                self.reboot_motor(dxl_id)
                # reboot_motor handles re-enabling torque internally
